...
typedef struct
{
  ...
  unsigned long Size;     /*Tamano en BYTES del archivo*//*23345*/
  unsigned long Reserved;      /*Reservado. establecido en cero*//*6789*/
  unsigned long OffSet;   /*Desplazamiento hacia datos*//*---------Inicio de los datos de la imagen----------------22*/
  unsigned long headsize; /*Tama�o de la cabecera del bitmap. establecido en 40.*/
  unsigned long Width;     /*Anchura en  pixeles.*//*18,19,20,21*/
  unsigned long Height;     /* Altura en pixeles.*//*22,23,24,25*/
...
int muestraBMP(int x, int y, char* NomArchivo)
{
    ...
    {
     fread(Datas, sizeof(unsigned char), Obj.Width/2, fp);/*para situar el comienzo de lectura del fichero*/
     c=0;
     in=0;
